(function (){
	...
	app.controller('sideBarCtrl', ['$rootScope', '$scope', '$http', 'postsService', '$location', '$routeParams',
        ...
        $scope.isSelected = function (item, filter) {
			...
			if (filter === 'posts') {
				result = postsService.labelText === 'allPosts';
			}
		...
		$rootScope.$on('$routeChangeSuccess', function (e, curr, prev) {
			...
			//$on event catch the change in the url according to the label clicked
			...
			if (curr.params.search) {
				postsService.labelText = curr.params.search;
				postsService.typeOfSearch = 'filterBySearch';
			}
